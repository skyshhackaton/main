...
import app.viz_per_model as vpm  # noqa: E402
...
def test_generate_per_model_charts(tmp_path, monkeypatch):
    # 테스트 속도를 위해 epochs/horizon 축소
    monkeypatch.setitem(vpm.LSTM_CFG, "epochs", 5)
    monkeypatch.setattr(vpm, "HORIZONS", (1, 7))
    csv_path = tmp_path / "hist.csv"
    _write_csv(csv_path, _make_candles())

    paths = vpm.generate(csv_path, market="KRW-BTC", out_dir=tmp_path, train_ratio=0.7)

    assert set(paths) == {"KNN pattern", "XGBoost", "LSTM"}
